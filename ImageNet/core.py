...
from funcs import growth_funcs
from funcs import prune_funcs
from funcs import redistribution_funcs
...
def add_sparse_args(parser):
    ...
    parser.add_argument(
        "--prune-rate", type=float, default=0.50, help="The pruning rate / prune rate."
    )
    parser.add_argument(
        "--density",
        type=float,
    ...
    parser.add_argument(
        "--dense", action="store_true", help="Enable dense mode. Default: False."
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
        help="Prints verbose status of pruning/growth algorithms.",
    )
    parser.add_argument(
        "--update_frequency",
        type=int,
    ...
    parser.add_argument(
        "--sparse_init", type=str, default="ER", help="sparse initialization"
    )
    parser.add_argument(
        "--multiplier",
        type=int,
...
class Masking(object):
    ...
    def __init__(
        ...
        # feed forward anchor weights
        self.fwd_anchor = kwargs.get("feed_forward", False)

    def init_optimizer(self):
        if "fp32_from_fp16" in self.optimizer.state_dict():
            for (name, tensor), tensor2 in zip(
                self.modules[0].named_parameters(),
                self.optimizer.state_dict()["fp32_from_fp16"][0],
            ):
                self.name_to_32bit[name] = tensor2
            self.half = True

    def init(self, mode="ER", density=0.05):
            ...
            for module in self.modules:
                for name, weight in module.named_parameters():
                    if name not in self.masks:
                        continue
                    self.masks[name][:] = (
                        (torch.rand(weight.shape) < density).float().data.cuda()
                    )
    ...
    def init_growth_prune_and_redist(self):
        if isinstance(self.growth_func, str) and self.growth_func in growth_funcs:
            ...
            print(
                "Use either a custom growth function or one of the pre-defined functions:"
            )
        ...
        if isinstance(self.prune_func, str) and self.prune_func in prune_funcs:
            ...
            print(
                "Use either a custom prune function or one of the pre-defined functions:"
            )
        ...
        if (
            isinstance(self.redistribution_func, str)
            and self.redistribution_func in redistribution_funcs
        ):
            self.redistribution_func = redistribution_funcs[self.redistribution_func]
        elif isinstance(self.redistribution_func, str):
            print("=" * 50, "ERROR", "=" * 50)
            print(
                "Redistribution mode function not known: {0}.".format(
                    self.redistribution_func
                )
            )
            print(
                "Use either a custom redistribution function or one of the pre-defined functions:"
            )
    ...
    def get_jacobian(self, func, inputs, create_graph=False, strict=False):
        ...
        for i, (input, target) in enumerate(prefetched_loader(inputs, fp16=False)):
            if i == 0:
                ...
                jacobian_4d = torch.autograd.functional.jacobian(
                    func, input_var, create_graph=create_graph, strict=strict
                )  # 100, 10, 100, 784
                print(jacobian_4d.size())
                jacobian_4d = jacobian_4d.view(64, 1000, 64, -1)
                jacobian_3d = torch.zeros(
                    jacobian_4d.size(0), jacobian_4d.size(1), jacobian_4d.size(3)
                )  # 100, 10, 784
    ...
    def approximnate_isometry(self):
        ...
        for e in range(1, 10001):
            ...
            for name in approxed_paras:
                ...
                layer_loss[name] = torch.norm(
                    torch.matmul(
                        torch.transpose(para.view(-1, para.size()[1]), 0, 1),
                        para.view(-1, para.size()[1]),
                    )
                    - torch.eye(para.size()[1]).cuda()
                )
                ...
                for name, weight in module.named_parameters():
                    ...
                    if self.redistribution_mode == "none":
                        new_mask = self.growth_func(
                            ...
                            name,
                            new_mask,
                            math.floor(self.name2removed[name]),
                            weight,
                        )
                        if self.args.grow_switch:
                            reini_value = float(
                                torch.mean(
                                    torch.abs(
                                        weight[
                            ...
                            num = (new_mask - self.masks[name].data.byte()).sum().item()
                            with torch.no_grad():
                                # weight[(new_mask - self.masks[name].data.byte())] = torch.FloatTensor(
                                #     int(num)).uniform_(-reini_value, reini_value).cuda()
                                weight[(new_mask - self.masks[name].data.byte())] = (
                                    torch.sign(
                                        grad[(new_mask - self.masks[name].data.byte())]
                                    )
                                    * torch.FloatTensor(int(num))
                                    .uniform_(0, reini_value)
                                    .cuda()
                                )
