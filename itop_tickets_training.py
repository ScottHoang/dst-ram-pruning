...
def get_ramanujan_scores(model, fn=th.abs, use_grad=False):
    ...
    for name, m in model.named_modules():
        if hasattr(m, "weight_orig"):
            ...
            if use_grad:
                ...
                weight = m.weight_orig.grad.data
...
def generate_characteristics(model, file):
    data = load_state_dict(file)
    state_dict = data["state_dict"]
    model.load_state_dict(state_dict)
    check_sparsity(model)
    return get_ramanujan_scores(model), data["epoch"]
