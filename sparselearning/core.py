...
def add_sparse_args(parser):
    ...
    parser.add_argument(
        "--sparse_init", type=str, default="ERK", help="sparse initialization"
    )
    parser.add_argument(
        "--growth",
        type=str,
...
class Masking(object):
    ...
    def init(self, mode="ERK", density=0.05, erk_power_scale=1.0):
        ...
        if mode == "GMP":
            ...
            for module in self.modules:
                for name, weight in module.named_parameters():
                    if name not in self.masks:
                        continue
                    self.masks[name] = torch.ones_like(
            ...
            for name, mask in self.masks.items():
                ...
                self.masks[name][:] = (
                    (torch.rand(mask.shape) < density_dict[name]).float().data.cuda()
                )
    ...
    def truncate_weights_GMP(self, epoch):
        ...
        total_prune_epochs = (
            self.args.multiplier * self.args.final_prune_epoch
            - self.args.multiplier * self.args.init_prune_epoch
            + 1
        )
        if (
            epoch >= self.args.multiplier * self.args.init_prune_epoch
            and epoch <= self.args.multiplier * self.args.final_prune_epoch
        ):
            prune_decay = (
                1
                - (
        ...
        print(
            "Total parameters under sparsity level of {0}: {1} after epoch of {2}".format(
                self.density, sparse_size / total_size, epoch
            )
        )
